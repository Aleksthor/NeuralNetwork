...
public class NeuralNetwork 
{
    ...
    public void GeneticAlgorithm()
    {
        ...
        for (int n = 0; n < hidden_layers.Count; n++) 
        {
            for (int i = 0; i < hidden_layers[n].rows; i++)
            {
                ...
                {
                    hidden_layers[n].Mutate(i, 0, Random.Range(2, 25));
                }
            }
        }
        ...
        for (int n = 0; n < hidden_weights.Count; n++)
        {
            for (int i = 0; i < hidden_weights[n].rows; i++)
            {
                ...
                for (int j = 0; j < hidden_weights[n].cols; j++)
                {
                    ...
                    if (check < random_mutation_chance)
                    {
                        hidden_weights[n].Mutate(i, j, Random.Range(2, 25));
                    }
                }
            }
        }
        ...
        for (int i = 0; i < input_hidden_weights.rows; i++)
        {
            ...
            for (int j = 0; j < input_hidden_weights.cols; j++)
            {
                ...
                if (check < random_mutation_chance)
                {
                    input_hidden_weights.Mutate(i, j, Random.Range(2, 25));
                }
            }
        }
        for (int i = 0; i < hidden_output_weights.rows; i++)
        {
            ...
            for (int j = 0; j < hidden_output_weights.cols; j++)
            {
                ...
                if (check < random_mutation_chance)
                {
                    hidden_output_weights.Mutate(i, j, Random.Range(2, 25));
                }
            }
        }
        for (int n = 0; n < hidden_biases.Count; n++)
        {
            for (int i = 0; i < hidden_biases[n].rows; i++)
            {
                ...
                for (int j = 0; j < hidden_biases[n].cols; j++)
                {
                    ...
                    if (check < random_mutation_chance)
                    {
                        hidden_biases[n].Mutate(i, j, Random.Range(2, 25));
                    }
                }
            }
        }
        for (int i = 0; i < output_bias.rows; i++)
        {
            ...
            for (int j = 0; j < output_bias.cols; j++)
            {
                ...
                if (check < random_mutation_chance)
                {
                    output_bias.Mutate(i, j, Random.Range(2,25));
                }
            }
        }
    ...
    public void WriteToFile(string path)
    {
        ...
        // Print all sizes first

        // Weights

        //  - Input - Hidden
        int input_hidden_rows = input_hidden_weights.rows;
        int input_hidden_cols = input_hidden_weights.cols;

        //  - All Hidden Weights
        int hidden_rows = hidden_weights[0].rows;
        int hidden_cols = hidden_weights[0].cols;

        //  - Hidden - Outputs 
        int hidden_output_rows = hidden_output_weights.rows;
        int hidden_output_cols = hidden_output_weights.cols;

        // Bias'

        //  - Hidden
        int hidden_bias_rows = hidden_biases[0].rows;
        int hidden_bias_cols = hidden_biases[0].cols;

        //  - Output
        int output_bias_rows = output_bias.rows;
        int output_bias_cols = output_bias.cols;


        // Write Weight Sizes
        writer.WriteLine(input_hidden_rows.ToString() + "," + input_hidden_cols.ToString());
        writer.WriteLine(hidden_rows.ToString() + "," + hidden_cols.ToString());
        writer.WriteLine(hidden_output_rows.ToString() + "," + hidden_output_cols.ToString());

        // Write Bias'
        writer.WriteLine(hidden_bias_rows.ToString() + "," + hidden_bias_cols.ToString());
        writer.WriteLine(output_bias_rows.ToString() + "," + output_bias_cols.ToString());

        // Write the Weight Matrix
        writer.WriteLine("InputHiddenWeights");
        for (int i = 0; i < input_hidden_weights.rows; i++)
        {
            float[] row = input_hidden_weights.GetRow(i);
            string line = "";
        ...
        writer.WriteLine("HiddenWeights");
        writer.WriteLine(hidden_weights.Count.ToString());
        for (int n = 0; n < hidden_weights.Count; n++)
        {
            writer.WriteLine("HiddenWeight" + n.ToString());
            for (int i = 0; i < hidden_weights[n].rows; i++)
            {
                float[] row = hidden_weights[n].GetRow(i);
                string line = "";
                for (int j = 0; j < row.Length; j++)
                {
                    line += row[j].ToString();
        ...
        writer.WriteLine("HiddenOutputWeights");
        for (int i = 0; i < hidden_output_weights.rows; i++)
        {
            float[] row = hidden_output_weights.GetRow(i);
            string line = "";
            for (int j = 0; j < row.Length; j++)
            {
                line += row[j].ToString();
        ...
        // Write Bias'
        writer.WriteLine("HiddenBias'");
        writer.WriteLine(hidden_biases.Count.ToString());
        for (int n = 0; n < hidden_biases.Count; n++)
        {
            writer.WriteLine("HiddenBias" + n.ToString());
            for (int i = 0; i < hidden_biases[n].rows; i++)
            {
                float[] row = hidden_biases[n].GetRow(i);
                string line = "";
                for (int j = 0; j < row.Length; j++)
                {
                    line += row[j].ToString();
        ...
        writer.WriteLine("OuputBias'");
        for (int i = 0; i < output_bias.rows; i++)
        {
            float[] row = output_bias.GetRow(i);
            string line = "";
            for (int j = 0; j < row.Length; j++)
            {
                line += row[j].ToString();
    ...
    public void ReadFromFile(string path)
    {
        ...
        int input_hidden_rows = int.Parse(numbers[0]);
        int input_hidden_cols = int.Parse(numbers[1]);

        //  - All Hidden Weights
        line = reader.ReadLine();
        numbers = line.Split(',');
        int hidden_rows = int.Parse(numbers[0]);
        int hidden_cols = int.Parse(numbers[1]);

        //  - Hidden - Outputs 
        line = reader.ReadLine();
        numbers = line.Split(',');
        int hidden_output_rows = int.Parse(numbers[0]);
        int hidden_output_cols = int.Parse(numbers[1]);
        ...
        line = reader.ReadLine();
        numbers = line.Split(',');
        int hidden_bias_rows = int.Parse(numbers[0]);
        int hidden_bias_cols = int.Parse(numbers[1]);

        //  - Output
        line = reader.ReadLine();
        numbers = line.Split(',');
        int output_bias_rows = int.Parse(numbers[0]);
        int output_bias_cols = int.Parse(numbers[1]);

        


        // Read the Weight Matrixes
        string title = reader.ReadLine();
        if (title != "InputHiddenWeights")
        {
            Debug.Log("InputHiddenWeights - Wrong Title");
            return;
        }
        input_hidden_weights = new Matrix(input_hidden_rows, input_hidden_cols);
        for (int i = 0; i < input_hidden_rows; i++)
        {
            string row = reader.ReadLine();
            numbers = row.Split(",");
            for (int j = 0; j < numbers.Length; j++)
            {
                input_hidden_weights.mat[i][j] = float.Parse(numbers[j]);
            }
        }

        title = reader.ReadLine();
        if (title != "HiddenWeights")
        {
            Debug.Log("HiddenWeights - Wrong Title");
            return;
        }
        string num = reader.ReadLine();
        int num_hidden = int.Parse(num);
        hidden_weights = new List<Matrix>();
        for (int n = 0; n < num_hidden; n++)
        {
            hidden_weights.Add(new Matrix(hidden_rows, hidden_cols));
            title = reader.ReadLine();
            if (title != "HiddenWeight" + n.ToString())
            {
                Debug.Log("HiddenWeight" + n.ToString() + " - Wrong Title");
                return;
            }
            for (int i = 0; i < hidden_rows; i++)
            {
                ...
                for (int j = 0; j < numbers.Length; j++)
                {
                    hidden_weights[n].mat[i][j] = float.Parse(numbers[j]);
                }
            }
        }
        ...
        title = reader.ReadLine();
        if (title != "HiddenOutputWeights")
        {
            Debug.Log("HiddenOutputWeights - Wrong Title");
            return;
        }
        hidden_output_weights = new Matrix(hidden_output_rows, hidden_output_cols);
        for (int i = 0; i < hidden_output_rows; i++)
        {
            string row = reader.ReadLine();
            numbers = row.Split(",");
            for (int j = 0; j < numbers.Length; j++)
            {
                hidden_output_weights.mat[i][j] = float.Parse(numbers[j]);
            }
        }
        ...
        if (title != "HiddenBias'")
        {
            Debug.Log("HiddenBias' - Wrong Title");
            return;
        }
        num = reader.ReadLine();
        num_hidden = int.Parse(num);
        hidden_biases = new List<Matrix>();
        for (int n = 0; n < num_hidden; n++)
        {
            hidden_biases.Add(new Matrix(hidden_rows, hidden_cols));
            title = reader.ReadLine();
            if (title != "HiddenBias" + n.ToString())
            {
                Debug.Log("HiddenBias" + n.ToString() + " - Wrong Title");
                return;
            }
            for (int i = 0; i < hidden_rows; i++)
            {
                ...
                for (int j = 0; j < numbers.Length; j++)
                {
                    hidden_biases[n].mat[i][j] = float.Parse(numbers[j]);
                }
            }
        }
        ...
        if (title != "OuputBias'")
        {
            Debug.Log("OuputBias' - Wrong Title");
            return;
        }
        output_bias = new Matrix(hidden_output_rows, hidden_output_cols);
        for (int i = 0; i < hidden_output_rows; i++)
        {
            string row = reader.ReadLine();
            numbers = row.Split(",");
            for (int j = 0; j < numbers.Length; j++)
            {
                output_bias.mat[i][j] = float.Parse(numbers[j]);
            }
        }
    ...
    public List<Matrix> HiddenLayers()
    {
        return hidden_layers;
    }

    public Matrix OutputLayer()
    {
        return output_layer;
    }
