...
public class Car : MonoBehaviour
{
    ...
    void UpdateTimeAlive()
    {
        ...
        if (timeWithoutCheckpoint > despawnTime)
        {
            isAlive = false;
            return;
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "Checkpoint")
        {
            ColliderIndex checpointAmount = other.GetComponent<ColliderIndex>();
            timeWithoutCheckpoint = 0;

            if (checpointAmount.index > current_Checkpoint + 2)
            {
                current_Checkpoint = 0;
            ...
            else if (checpointAmount.index == current_Checkpoint - 1)
            {
                isAlive = false;
                return;
            }
            else if (checpointAmount.index >= 35)
            {
                isAlive = false;
                return;
            }
            else if (checpointAmount.index == 35 && current_Checkpoint == 1)
            {
                isAlive = false;
                return;
            }
            else if (checpointAmount.index == 35 && current_Checkpoint == 0)
            {
                isAlive = false;
                return;
            }
            ...
            {
                current_Checkpoint = checpointAmount.index;
            }

            //Update Checkpoint Index
            //if (checpointAmount.index == current_Checkpoint)
            //{
            //    //Reset Checkpoint and add to Lap when car have reached all Checkpoints
            //    if (checpointAmount.index == current_Checkpoint - 1)
            //    {
            //        current_Checkpoint = 0;
            //        current_Lap++;
            //    }

            //    //Add to Checkpoint when passing one
            //    else
            //    {
            //        current_Checkpoint++;
            //    }
